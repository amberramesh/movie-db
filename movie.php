...
function manage_cookies()
{
  ...
  if(isset($_SESSION['user_id']))
  {
    ...
    {
      //first value in comma-separated cookie value is user id.
      setcookie($cookie_name,$movieid ,time() + (86400 * 30), '/');
      return;
    }
    $cookie_arr = explode(',',$_COOKIE[$cookie_name]);
    $pos = array_search($movieid, $cookie_arr);
    if($pos === false)
    {
      if(count($cookie_arr) == 8)
        array_splice($cookie_arr, 0, 1);
    }
    else {
      array_splice($cookie_arr, $pos, 1);
    }
